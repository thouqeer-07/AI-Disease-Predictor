...
class ExcelReporter {
  async onRunComplete(_testContexts, results) {
    ...
    const fileLocation = path.join(reportDir, 'selenium-report.xlsx');
    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet('Selenium Test Report');
    ...
    results.testResults.forEach((suiteResult) => {
      const suiteCategory = suiteResult.testResults[0]?.ancestorTitles[0] || 'Selenium End-to-End Tests';
      suiteResult.testResults.forEach((tr) => {
        const titleMatch = tr.title.match(/(E2E_\d{3}):\s*(.*)/);
    ...
    console.log(`\n📊 Excel report successfully generated with ${allTestItems.length} selenium test cases at:\n   ${fileLocation}\n`);
  }
}
