...
class ExcelReporter {
  async onRunComplete(_testContexts, results) {
    ...
    const fileLocation = path.join(reportDir, 'baseline-load-report.xlsx');
    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet('Baseline & Load Performance Report');
    ...
    results.testResults.forEach((suiteResult) => {
      const suiteCategory = suiteResult.testResults[0]?.ancestorTitles[0] || 'Baseline & Load Performance Tests';
      suiteResult.testResults.forEach((tr) => {
        const titleMatch = tr.title.match(/((?:PERF_|LOAD_)\d{3}):\s*(.*)/);
    ...
    console.log(`\n📊 Excel report successfully generated with ${allTestItems.length} baseline and load test cases at:\n   ${fileLocation}\n`);
  }
}
