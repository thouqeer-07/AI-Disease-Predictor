...
const Dashboard = () => {
 ...
 const navigate = useNavigate();
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // 7. Fetch Medications for Refill Alerts
  ...
  useEffect(() => {
    ...
    const dashChannel = supabase
      .channel('realtime:dashboard_metrics')
 ...
 <div className="flex items-center gap-4 relative">
    ...
    {showNotifications && (
      <div className="absolute top-14 right-0 w-80 bg-white border border-slate-100 shadow-xl rounded-xl p-4 z-50">
        <div className="flex items-center gap-2 mb-4 text-orange-500">
          <AlertCircle className="w-5 h-5" />
          <h3 className="font-bold">Refill Alerts</h3>
        </div>
        {lowStockMeds.length > 0 ? (
          <div className="flex flex-col gap-3">
            {lowStockMeds.map((med) => (
              <div key={med.id} className="p-3 rounded-lg bg-orange-50/50 border border-orange-100 flex justify-between items-center group">
                <div>
                  <p className="font-bold text-slate-900 text-sm group-hover:text-orange-600 transition-colors">{med.name}</p>
                  <p className="text-xs text-orange-600 font-medium">{med.stock_count} doses left</p>
                </div>
                <Button 
                  variant="primary" 
                  size="sm" 
                  className="rounded-lg h-7 px-3 text-[10px] font-bold shadow-sm bg-orange-500 hover:bg-orange-600 border-none text-white shrink-0"
                  onClick={async () => {
                    try {
          ...
          <p className="text-sm text-slate-400 text-center py-4">No alerts right now.</p>
        )}
      </div>
    )}
 ...
 <span className="text-xs text-slate-500">{dailyScore.label}</span>
  ...
  {/* Doctor Chats */}
  <Card>
    <CardHeader title="Doctor Chats" icon={MessageCircle} />
    <div className="flex flex-col gap-4">
      {appointments.length > 0 ? (
        appointments.map((appt) => (
          <div key={appt.id} className="p-4 rounded-xl bg-slate-50 dark:bg-zinc-800/40 border border-slate-100 dark:border-zinc-800 flex items-center justify-between gap-4 group hover:border-primary/50 transition-all duration-300">
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                <p className="font-bold text-slate-900 dark:text-white truncate group-hover:text-primary transition-colors">Dr. {appt.doctor_name}</p>
                <p className="text-xs text-slate-500 dark:text-slate-400 truncate">{appt.specialization || 'Specialist'}</p>
              </div>
            </div>
            <Button 
              variant="primary" 
              size="sm" 
              className="rounded-xl px-4 h-9 text-xs font-bold shrink-0 shadow-md"
        ...
        <div className="text-center py-6 text-slate-400">
          ...
          <p className="text-xs">No active approved chats.</p>
        </div>
      )}
    </div>
  </Card>
 ...
 Start Chat
  ...
  {/* AI Insights - Rectangle shape at the bottom */}
  <div className="lg:col-span-3">
    <Card>
      <CardHeader title="AI Insights" icon={Brain} />
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {insights.length > 0 ? insights.map((insight) => (
          <div key={insight.id} className={`flex gap-3 p-4 rounded-xl border ${
            insight.type === 'warning' ? 'bg-orange-50 border-orange-100' :
            insight.type === 'success' ? 'bg-emerald-50 border-emerald-100' :
            'bg-blue-50 border-blue-100'
          }`}>
            {insight.type === 'warning' ? <AlertCircle className="w-6 h-6 text-orange-600 shrink-0" /> :
             insight.type === 'success' ? <TrendingUp className="w-6 h-6 text-emerald-600 shrink-0" /> :
          ...
          <p className="text-sm text-slate-400 text-center py-4 md:col-span-3">No AI insights generated yet.</p>
        )}
      </div>
    </Card>
  </div>
