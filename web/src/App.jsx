...
function AppContent() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      if (session && session.user && (!session.user.email_confirmed_at || session.user.user_metadata?.aura_verified === false)) {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (_event, session) => {
      // Prevent auto-login immediately after registration
      const isNewSignup = session?.user?.created_at && (Date.now() - new Date(session.user.created_at).getTime() < 15000);

      if (isNewSignup || (session && session.user && (!session.user.email_confirmed_at || session.user.user_metadata?.aura_verified === false))) {
        await supabase.auth.signOut();
        if (isNewSignup) {
    ...
    <Router>
      <div className="min-h-screen bg-slate-50 dark:bg-zinc-950 transition-colors duration-300">
        <Routes>
          ...
          <Route path="/doctor-onboarding" element={<ProtectedRoute requireOnboarding={false}><DoctorOnboarding /></ProtectedRoute>} />
          <Route path="/patient-onboarding" element={<ProtectedRoute requireOnboarding={false}><PatientOnboarding /></ProtectedRoute>} />
